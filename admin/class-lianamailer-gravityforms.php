 ...
 * @link     https://www.lianatech.com
 */
class LianaMailer_GravityForms {
	/**
	 * REST API options to save
	 *
	 * @var lianamailer_gravityforms_options array
	 */
	private $lianamailer_gravityforms_options = array(
		'lianamailer_userid'     => '',
	...
	public function __construct() {
		add_action(
			'admin_menu',
			array( $this, 'lianamailer_gravity_forms_add_plugin_page' )
		);

		add_action(
			'admin_init',
			array( $this, 'lianamailer_gravity_forms_page_init' )
		);
	}
	 ...
	 * Add an admin page
	 */
	public function lianamailer_gravity_forms_add_plugin_page() {
		...
		if ( ! isset( $admin_page_hooks['lianamailer'] ) ) {
			add_menu_page(
				...
				'manage_options',
				'lianamailer',
				array( $this, 'lianamailer_gravity_forms_create_admin_page' ),
	 ...
	 */
	public function lianamailer_gravity_forms_create_admin_page() {
	 ...
	 */
	public function lianamailer_gravity_forms_page_init() {

		$page    = 'lianamailer_gravityforms_admin';
		$section = 'lianamailer_gravityforms_section';

		register_setting(
			'lianamailer_gravityforms_option_group',
			'lianamailer_gravityforms_options',
			array(
				$this,
				'lianamailer_gravity_forms_sanitize',
			)
		);
		...
		add_settings_section(
			$section,
			'',
			array( $this, 'lianamailer_gravity_forms_section_info' ),
			$page
		);
	 ...
	 * @return null
	 */
	public function lianamailer_gravity_forms_sanitize( $input ) {
	 ...
	 * Empty section info.
	 */
	public function lianamailer_gravity_forms_section_info() {
	...
	public function lianamailer_connection_check_callback() {
		...
		if ( ! empty( $this->lianamailer_gravityforms_options['lianamailer_userid'] ) || ! empty( $this->lianamailer_gravityforms_options['lianamailer_secret_key'] ) || ! empty( $this->lianamailer_gravityforms_options['lianamailer_realm'] ) ) {
			$rest = new Rest(
				$this->lianamailer_gravityforms_options['lianamailer_userid'],
				$this->lianamailer_gravityforms_options['lianamailer_secret_key'],
...
if ( is_admin() ) {
	$lianamailer_gravity_forms = new LianaMailer_GravityForms();
}
